...
Функция Листы() Экспорт
    Если НЕ ТипЗнч(Листы) = Тип("ТаблицаЗначений") Тогда
        ВызватьИсключение "Листы не прочитаны";
    КонецЕсли;

    Результат = Новый Соответствие();
    
    Для каждого СтрокаЛисты Из Листы Цикл
        Результат.Вставить(СтрокаЛисты.Номер, СтрокаЛисты.Имя);
    КонецЦикла;
...
Процедура ПрочитатьЛисты()
	...
	ФайлыЛистов = НайтиФайлыЛистов(КаталогЛистов);
	...
	Для Счетчик = 1 По ФайлыЛистов.Количество() Цикл
		...
		НовыйЛист.Лист = Новый ЛистЭксель(НовыйЛист.Имя, Счетчик, ФайлыЛистов[Счетчик], ЭтотОбъект);	
		НовыйЛист.Лист.Прочитать();
	КонецЦикла;
	
КонецПроцедуры

Функция НайтиФайлыЛистов(КаталогЛистов)
	ФайлыЛистов = НайтиФайлы(КаталогЛистов, "*.xml", Ложь);	
	ОписанияФайловЛистов = Новый Соответствие();

	Для Каждого ТекФайл Из ФайлыЛистов Цикл
		ОписанияФайловЛистов.Вставить(Число(СтрЗаменить(ТекФайл.ИмяБезРасширения, "sheet", "")), ТекФайл.ПолноеИмя);	
	КонецЦикла;
	
	Возврат ОписанияФайловЛистов;
КонецФункции
